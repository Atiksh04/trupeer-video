...
export default function VideoCanvas({ videoRef, video, zoomBlocks, isPlaying, handlePlayPause, handleTimeUpdate, currentTime }) {
  ...
  useEffect(() => {
    ...
    const drawFrame = () => {
      ...
      const activeZoomBlock = zoomBlocks.find(block => 
      ...
      if (activeZoomBlock) {
        ...
        const sourceX = Math.max(0, Math.min(x - zoomWidth / 2, canvas.width - zoomWidth))
        const sourceY = Math.max(0, Math.min(y - zoomHeight / 2, canvas.height - zoomHeight))

        // Draw the zoomed portion
        ctx.drawImage(
          videoElement,
          sourceX, sourceY, zoomWidth, zoomHeight,
  ...
  return (
    <div className="mb-4 relative">
      ...
      <div className="absolute bottom-4 left-4 space-x-2">
        ...
        >
          <img src={ isPlaying ? PauseIcon : PlayIcon } alt="play-icon" className='h-4 w-4 fill-white'/>
        </button>
      </div>
    </div>
  )
